...
from core.tubes import Tube, GroupTube
...
class SynopsisRenderer:
    ...
    def _composite_group(self, canvas: np.ndarray, group: GroupTube,
                         local_idx: int, global_frame: int):
        """Composite all sub-tubes of a GroupTube at the given local frame index."""
        for sub_tube in group.sub_tubes:
            # Map from the group's timeline to each sub-tube's timeline
            group_global = group.start_frame + local_idx
            sub_local = group_global - sub_tube.start_frame
            
            if 0 <= sub_local < sub_tube.duration:
                bbox = sub_tube.bboxes[sub_local]
                frame = sub_tube.frames[sub_local]
                mask = sub_tube.masks[sub_local] if sub_tube.masks else None
                self._composite_object(canvas, frame, bbox, mask)
        
    def render(self, placements: List[Tuple], 
              output_path: str, add_metadata: bool = True):
        if not placements:
            raise ValueError("No placements to render")
        
        max_frame = max([start_time + tube.bboxes.__len__()
        ...
        for frame_idx in range(max_frame):
            ...
            for tube, start_time in placements:
                ...
                if 0 <= local_idx < len(tube.bboxes):
                    if isinstance(tube, GroupTube):
                        # Composite all sub-tubes individually
                        self._composite_group(canvas, tube, local_idx, frame_idx)
                    else:
                    ...
                    if add_metadata:
                        ...
                        x2, y2 = min(self.width, bbox[2]), min(self.height, bbox[3])
    ...
    def render_with_blending(self, placements: List[Tuple], 
        ...
        for frame_idx in range(max_frame):
            ...
            for tube, start_time in placements:
                ...
                if 0 <= local_idx < len(tube.bboxes):
                    if isinstance(tube, GroupTube):
                        # Composite each sub-tube individually with blending
                        for sub_tube in tube.sub_tubes:
                            group_global = tube.start_frame + local_idx
                            sub_local = group_global - sub_tube.start_frame
                            if 0 <= sub_local < sub_tube.duration:
                                self._blend_object(
                                    canvas, sub_tube.frames[sub_local],
                                    sub_tube.bboxes[sub_local],
                                    sub_tube.masks[sub_local] if sub_tube.masks else None,
                                    alpha)
                    else:
                        self._blend_object(
                            canvas, tube.frames[local_idx],
                            tube.bboxes[local_idx],
                            tube.masks[local_idx] if tube.masks else None,
                            alpha)
    ...
    def _blend_object(self, canvas, frame, bbox, mask, alpha):
        """Alpha-blend an object onto a float canvas."""
        x1, y1, x2, y2 = bbox
        x1, y1 = max(0, x1), max(0, y1)
        x2, y2 = min(self.width, x2), min(self.height, y2)
